...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      ...
      // Stale-while-revalidate for HTML pages (serve stale instantly, regenerate in background)
      {
        source: '/((?!_next|static|api).*)',
        headers: [
          {
            key: 'Cache-Control',
            value: 'public, s-maxage=172800, stale-while-revalidate=604800',
          },
        ],
      },
  ...
  // URL Redirects for legacy SEO paths
  async redirects() {
    return [
      {
        source: '/:brand/:model/variant/:variantSlug',
        destination: '/:brand/:model/:variantSlug',
        permanent: true, // 301 Redirect for SEO
      },
      {
        // Handle any legacy /variants/slug paths
        source: '/variants/:slug',
        destination: '/', // Redirects to home or search, since we don't have brand/model here easily
        permanent: true,
      },
      // 301 redirect: old /price-in/{city} subdirectory → flat /price-in-{city} (CarWale-style)
      {
        source: '/:brand-cars/:model/price-in/:city',
        destination: '/:brand-cars/:model/price-in-:city',
        permanent: true, // 301 for SEO consolidation
      },
      // 404 Fixes based on GSC Report
      {
        source: '/news/kia-clavis-vs-competitors-detailed-comparison-fc0d1181',
        destination: '/', // Deleted news article -> Home
        permanent: true,
      },
      {
        source: '/jeep-cars',
        destination: '/', // Discontinued brand -> Home
        permanent: true,
      },
      {
        source: '/citroen-cars',
      ...
      {
        source: '/unknown-cars',
        destination: '/', // Error URL -> Home
        permanent: true,
      },
      {
        source: '/maruti-suzuki-cars/e-vitara/(images|variants)',
        destination: '/maruti-suzuki-cars/e-vitara', // Fallback to main model page
        permanent: true,
      },
      {
        source: '/hyundai-cars/tucson/images',
        destination: '/hyundai-cars/tucson', // Fallback to main model page
        permanent: true,
      },
      {
        source: '/maruti-suzuki-cars/ciaz/variants',
        destination: '/maruti-suzuki-cars/ciaz', // Fallback to main model page
        permanent: true,
      }
    ]
  },

  // URL rewrites: API proxy only (price-in pages now handled natively by [variant] route)
  async rewrites() {
    return [
      {
        source: '/api/:path*',
