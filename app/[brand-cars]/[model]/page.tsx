...
export async function generateMetadata({ params }: ModelPageProps): Promise<Metadata> {
  ...
  // Try to fetch real prices for CTR-optimized titles
  let seoOptions: { startingPrice?: number; endingPrice?: number; variantCount?: number; bodyType?: string; fuelTypes?: string[] } | undefined
  try {
    ...
    if (brandsRes.ok && modelsWithPricingRes.ok) {
      ...
      const modelsData = (await modelsWithPricingRes.json())?.data || []
      ...
      if (brand) {
        ...
        if (model) {
          seoOptions = {
            ...
            endingPrice: model.highestPrice || model.lowestPrice || 0,
            variantCount: model.variantCount || 0,
    ...
    // Fallback to basic SEO if fetch fails
  }
...
export default async function ModelPage({ params }: ModelPageProps) {
  ...
  // AUTO-GENERATE FAQs if backend has none — targets long-tail search queries
  if (formattedFaqs.length === 0) {
    const brandName = modelData.brand
    const modelName = modelData.name
    const year = new Date().getFullYear()
    const startPrice = modelData.startingPrice > 0 ? `₹${(modelData.startingPrice / 100000).toFixed(2)} Lakh` : 'competitive prices'
    const endPrice = modelData.endingPrice > 0 ? `₹${(modelData.endingPrice / 100000).toFixed(2)} Lakh` : ''
    const variantCount = modelData.variants?.length || 0
    const bodyType = modelData.bodyType || 'car'

    formattedFaqs = [
      {
        question: `What is the price of ${brandName} ${modelName} in India ${year}?`,
        answer: `The ${brandName} ${modelName} price in India starts at ${startPrice} (ex-showroom)${endPrice ? ` and goes up to ${endPrice}` : ''}. The on-road price varies by city due to different RTO and insurance charges.`
      },
      {
        question: `How many variants does ${brandName} ${modelName} have?`,
        answer: `The ${brandName} ${modelName} is available in ${variantCount > 0 ? variantCount : 'multiple'} variants across different fuel types and transmission options. Each variant offers a different set of features to suit different budgets and preferences.`
      },
      {
        question: `What is the on-road price of ${brandName} ${modelName} in Mumbai?`,
        answer: `The on-road price of ${brandName} ${modelName} in Mumbai includes ex-showroom price, RTO charges (Maharashtra), road tax, insurance, and other charges. Use our on-road price calculator for the exact price in your city.`
      },
      {
        question: `Is ${brandName} ${modelName} worth buying in ${year}?`,
        answer: `The ${brandName} ${modelName} is a strong contender in the ${bodyType} segment, offering good value with its features, build quality, and after-sales network. Consider comparing it with competitors to find the best fit for your needs.`
      },
      {
        question: `What is the EMI for ${brandName} ${modelName}?`,
        answer: `The EMI for ${brandName} ${modelName} starts at approximately ₹${modelData.emi?.starting ? modelData.emi.starting.toLocaleString('en-IN') : Math.round((modelData.startingPrice / 100000) * 1000).toLocaleString('en-IN')} per month for a 60-month loan at standard interest rates. Use our EMI calculator for exact figures based on your down payment and loan tenure.`
      },
      {
        question: `Which variant of ${brandName} ${modelName} is the best to buy?`,
        answer: `The best variant depends on your budget and needs. For value buyers, the base or mid variant offers essential features at a lower price. For feature-rich experience, the top variants include premium features like sunroof, advanced infotainment, and enhanced safety.`
      }
    ]
  }
  ...
  // Generate breadcrumb schema for rich results
  const brandDisplayName = modelData.brand
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', item: '/' },
    { name: `${brandDisplayName} Cars`, item: `/${resolvedParams['brand-cars']}` },
    { name: modelData.name, item: `/${resolvedParams['brand-cars']}/${resolvedParams.model}` },
  ])
