...
export function registerRoutes(app: Express, storage: IStorage, backupService?: BackupService) {
  ...
  app.get("/api/brands", publicLimiter, redisCacheMiddleware(RedisCacheTTL.BRANDS), async (req, res) => {
    try {
      ...
      // `admin=true` bypasses the price filter so all active brands show in the admin panel
      const isAdminRequest = req.query.admin === 'true';
      const applyPriceFilter = !includeInactive && !isAdminRequest;
      const brands = await storage.getBrands(includeInactive, applyPriceFilter);
