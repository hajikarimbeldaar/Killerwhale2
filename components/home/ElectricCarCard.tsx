...
export default function ElectricCarCard({ car, index, onClick }: ElectricCarCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="flex-shrink-0 w-[240px] sm:w-64 bg-white rounded-xl border border-emerald-200 hover:border-emerald-400 hover:shadow-lg active:scale-95 transition-all duration-300 overflow-hidden cursor-pointer group"
        >
            {/* Image Container - Green tint background */}
            <div className="relative h-36 sm:h-44 bg-gradient-to-br from-emerald-50 to-green-100 overflow-hidden">
                {/* New/Popular Badge */}
                {car.isNew && (
            ...
            <div className="p-4 sm:p-5">
                ...
                <div className="flex flex-col mb-3 sm:mb-4">
                    <div className="flex items-baseline">
                        <span className="text-emerald-600 font-bold text-base sm:text-lg">₹ {(displayPrice / 100000).toFixed(2)} Lakh</span>
                ...
                <button
                    className="w-full bg-gradient-to-r from-emerald-600 to-green-500 hover:from-emerald-700 hover:to-green-600 text-white py-2 sm:py-2.5 rounded-lg font-semibold text-xs sm:text-sm transition-all duration-200 shadow-md hover:shadow-lg transform group-hover:scale-105"
                >
                    View Details
                </button>
            </div>
