...
export default function UpcomingCarCard({ car, index, onClick }: UpcomingCarCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="flex-shrink-0 w-[240px] sm:w-64 bg-white rounded-xl border border-gray-200 hover:shadow-lg active:scale-95 transition-all duration-300 overflow-hidden cursor-pointer group"
        >
            {/* Image Container */}
            <div className="relative h-36 sm:h-44 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                ...
                <div className="w-full h-full flex items-center justify-center relative">
                    {car.image ? (
                        <OptimizedImage
                            src={car.image}
                            alt={`${car.brandName} ${car.name} price in India - Expected starting at ₹${(car.expectedPriceMin / 100000).toFixed(2)} Lakh`}
                            fill
