    ...
    <section className={`${backgroundClasses[background]} py-6`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className={`text-2xl sm:text-3xl font-bold ${background === 'gradient' ? 'text-white' : 'text-gray-900'} mb-2`}>
              {title}
            </h1>
            {subtitle && (
              <p className={`text-sm sm:text-base ${background === 'gradient' ? 'text-blue-100' : 'text-gray-600'}`}>
                {subtitle}
              </p>
            )}
          </div>
