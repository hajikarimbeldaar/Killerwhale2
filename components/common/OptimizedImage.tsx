...
export const OptimizedImage = forwardRef<HTMLImageElement, OptimizedImageProps>(({
    ...
    // For priority (LCP) images: render immediately at full opacity, no skeleton
    // For lazy images: use the fade-in animation for a smooth experience
    ...
    return (
        <div className={`relative ${fill ? 'w-full h-full' : ''} ${wrapperClassName}`}>
            <Image
                ...
                className={`${className} ${isPriority ? '' : (isLoading ? 'opacity-0' : 'opacity-100')} transition-opacity duration-300`}
                {...props}
            />
            {/* Loading skeleton — skipped for priority/LCP images */}
            {!isPriority && (
                <div
