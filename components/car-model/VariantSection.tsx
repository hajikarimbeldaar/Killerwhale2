...
export default function VariantSection({ carData }: VariantSectionProps) {
  ...
  const VariantCard = ({ variant }: { variant: typeof variants[0] }) => (
    ...
    >
      ...
      {/* Action Button */}
      <button 
        className="w-full bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white py-2.5 px-4 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-center group-hover:shadow-md"
        onClick={(e) => {
          e.stopPropagation()
