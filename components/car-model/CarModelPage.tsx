...
export default function CarModelPage({ model, initialVariants = [], newsSlot }: CarModelPageProps) {
  ...
  // Deterministic ratings to perfectly match the ModelOwnerReviews component below
  const carFullName = `${model?.brand || 'Brand'} ${model?.name || 'Model'}`
  const nameHash = carFullName.split('').reduce((a, c) => a + c.charCodeAt(0), 0)

  // Use actual API ratings if they exist, otherwise generate the exact same fallback as the reviews section
  const hasRealReviews = reviews.length > 0 || (model.rating !== undefined && model.rating > 0)
  const displayRating = hasRealReviews
    ? averageRating.toFixed(1)
    : (4.0 + (nameHash % 10) / 10).toFixed(1)

  const displayReviewsCount = hasRealReviews
    ? totalReviews
    : 45 // The ModelOwnerReviews generates exactly 45 reviews
  ...
  // Engine options — populated from backend data, empty fallback
  const engineOptions: any[] = []

  // Mileage data — populated from backend
  ...
  // News articles and videos — populated from backend or BrandNews component
  const newsArticles: any[] = []
  const featuredVideo: any = null
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                ...
                <div className="flex-1">
                  ...
                  {!model?.isUpcomingCar && (
                    <div className="flex items-center space-x-3 mb-4">
                      {/* Rating Badge - Styled exactly like Google Play / Car Dekho */}
                      <div className="flex items-center bg-gradient-to-r from-red-600 to-orange-500 text-white px-2.5 py-1 rounded shadow-sm text-sm">
                        ...
                        <span className="ml-1 text-white/90 font-medium">({displayReviewsCount})</span>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <button
                  onClick={() => {
                    ...
                    // Clear any stored variant preference so it defaults to base variant
                    localStorage.removeItem('priceBreakupVariant')
                  ...
                  className="w-full bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
                >
                  ...
                  <>
                    ...
                    >
                      <div className="flex items-center justify-center">
                        <div className="relative max-w-2xl w-full aspect-[16/9] mx-auto">
                          <OptimizedImage
      ...
      {/* SEO Internal Linking Mesh — text links for PageRank distribution */}
      {model && (
        <SEOInternalLinks
